...
if ($cfg->showAnsweredTickets()) {
	$nav->addSubMenu(
		array('desc' => $open_name . ' (' . number_format($stats['open'] + $stats['answered']) . ')',
	...
	if ($stats) {
		$nav->addSubMenu(
			array('desc' => $open_name . ' (' . number_format($stats['open']) . ')',
				'title' => __('Open Tickets'),
				'href' => 'tickets.php?status=open',
				'iconclass' => 'Ticket'),
			((!$_REQUEST['status'] && !isset($_SESSION['advsearch'])) || $_REQUEST['status'] == 'open')
	...
	if ($stats['answered']) {
		$nav->addSubMenu(
			array('desc' => __('Answered') . ' (' . number_format($stats['answered']) . ')',
...
if ($stats['assigned'] || $stats['teamAssigned'] > 0) {
	$nav->addSubMenu(
		array('desc' => __('My Groups Tickets') . ' (' . number_format($stats['assigned']) . ')',
...
if (isset($_SESSION['advsearch'])) {
	...
	$count = $tickets->count();
	$nav->addSubMenu(
		array('desc' => __('Search') . ' (' . number_format($count) . ')',
