...
<?php           if ($thisstaff->hasPerm(Email::PERM_BANLIST)) {
			...
			<div class="hidden">
				...
				<form action="" target="_blank" method="post" id="billSupportForm">
					...
					<input type="hidden" value="<?= $ticket->getLastMessageDate(); ?>" name="Prefill[data][0][date]">
			...
			<span class="billSupportButton action-button" data-placement="bottom" data-toggle="tooltip" title="Bill support"><i class="glyphicon glyphicon-eur"></i></span>
            ...
            <td><?php
				if ($label == 'PID') {
					...
					if (count($arrPid) > 1) {
						...
						echo Format::htmlchars(implode(' ', $arrLinks));
					} else {
						echo $v;
					}
				} else {
