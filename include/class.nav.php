...
class StaffNav {
	...
	public function __construct($staff, $panel = 'staff') {
		$this->staff = $staff;
	...
	public function setTabActive($tab, $menu = '') {
		if ($this->tabs[$tab]) {
	...
	public function setActiveTab($tab, $menu = '') {
	...
	public function setActiveSubMenu($mid, $tab = '') {
		if (is_numeric($mid)) {
			$this->activeMenu = $mid;
		} elseif ($mid && $tab && ($subNav = $this->getSubNav($tab))) {
			foreach ($subNav as $k => $menu) {
				if (strcasecmp($mid, $menu['href'])) {
					continue;
				}
	...
	public function addSubMenu($item, $active = false) {
	...
	public function getSubMenus() { //Private.
		global $cfg;
				...
				case 'tickets':
					...
					if ($staff) {
						if (($assigned = $staff->getNumAssignedTickets())) {
							$subnav[] = array('desc' => __('My Group Tickets ') . " ($assigned)",
								'href' => 'tickets.php?status=assigned',
								'iconclass' => 'assignedTickets',
								'droponly' => true);
						}

						$subnav[] = array('desc' => __('My tickets ') . "({$staff->getNumAssignedTickets()})",
							'href' => 'tickets.php?status=mxvp&mxvptype=user&mxvpid=' . $staff->getId(),
							'iconclass' => 'assignedTickets',
							'droponly' => true);
	...
	public function getSubMenu($tab = null) {
		$tab = $tab ? $tab : $this->activetab;
		return $this->submenus[$this->getPanel() . '.' . $tab];
	}

	public function getSubNav($tab = null) {
...
class UserNav {
	...
	public function __construct($user = null, $active = '') {
		$this->user = $user;
